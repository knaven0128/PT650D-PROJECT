...
Case comEvReceive 'OnComm was triggered because characters were received
    ...
    If Len(Weight) > 0 Then 'Did we find anything to display?
        Me.Text1.Text = CStr(Mid(Weight, Val(Me.txtstr.Text), Val(Me.txtlen.Text)))
        
        contain = InStr(1, Text1.Text, "kg", vbTextCompare)
        If contain = 0 Then
        Me.Text1.Text = ""
        End If
...
End If
    ...
    If Trim(Me.Text1.Text) = "" Then
             Me.Text1.Text = current
    Else
     Me.txtkilo.Text = Text1.Text
     symboln = InStr(1, txtkilo.Text, Trim(Me.txtnegative.Text), vbTextCompare)
     If symboln > 0 Then
     Me.txtkilo.Text = negative + ReturnNonAlpha(Me.txtkilo.Text)
     Else
     Me.txtkilo.Text = positive + ReturnNonAlpha(Me.txtkilo.Text)
     End If
     End If
End Sub
